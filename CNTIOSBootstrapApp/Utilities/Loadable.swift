...
typealias LoadableBinding<Value> = Binding<Loadable<Value>>
...
extension Publisher {
    ...
    func sinkToLoadable(_ loadableBinding: LoadableBinding<Output>) -> AnyCancellable {
        sink(receiveCompletion: { subscriptionCompletion in
            switch subscriptionCompletion {
            case let .failure(error):
                loadableBinding.wrappedValue = .failed(error)
            default: return
            }
        }, receiveValue: {
            loadableBinding.wrappedValue = .loaded($0)
        })
    }
}
